...
* 기본 Font와 Alignment를 MyDefaultWidgets와 맞춤
* TableWidget에서의 Tab, Shift+Tab(BackTab), Right, Left, Enter 키의 작동 재정의 (엑셀과 비슷하게)
"""


class MyItemLineEdit(MyLineEdit):
    CompleterRowCount = 5  # todo 임시값
    ...
    def __init__(self, item: QTableWidgetItem, parent=None):
        ...
        completer.popup().setFont(MyDefaultWidgets.basicQFont(point_size=MyDefaultWidgets.basicPointSize() - 1))
        completer.setCaseSensitivity(Qt.CaseInsensitive)  # 대소문자 무시
        completer.setMaxVisibleItems(MyItemLineEdit.CompleterRowCount)
        self.setCompleter(completer)

        table = self.__item.tableWidget()
        table.itemChanged.connect(self._myItemChanged)
        self.textChanged.connect(self._myTextChanged)  #todo 임시
    ...
    * event
    ...
    def focusInEvent(self, a0: QFocusEvent) -> None:
        ...
        row, col = self.item().row(), self.item().column()
        self.item().tableWidget().setCurrentCell(row, col)
    ...
    def keyPressEvent(self, event: QKeyEvent) -> None:
        ...
        if event.key() == Qt.Key_Right:
            ...
            self.item().tableWidget().setFocusCell(self.item().row() + 1, 1)
    ...
    slot
    * myItemChanged
    * myEditingFinished (override) 
    """
    @MyPyqtSlot(QTableWidgetItem)
    def _myItemChanged(self, item: QTableWidgetItem):
        # print('MyItemChanged:', item.text())
        if item == self.item():
            self.blockSignals(True)
            self.setText(item.text())
            self.blockSignals(False)

    @MyPyqtSlot(str)  # todo 한글 입력시 한글자가 씹히는 이유는 이 슬롯 때문임. setText와 setCursorPosition 모두 문제인듯 -> editingFinish로 옮겨봄, 문제없으면 삭제하기
    def _myTextChanged(self, s: str):
        # print('MyTextChanged:', s)
    ...
    #     print('MyEditingChanged')
    #     super()._myEditingFinished()
    #     cursor_pos = self.cursorPosition()
    #     self.blockSignals(True)
