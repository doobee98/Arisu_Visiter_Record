...
class MyModelView(QObject):
    ...
    * columnButtons
    ...
    def render(self, items: List[QTableWidgetItem], model: Type[MyModel] = None):
        """
    ...
    def _renderText(self, items: List[QTableWidgetItem], model: Type[MyModel]):
        if items and model:
            table = self.tableView()
            for model_field in table.modelFieldList():
                field_column = table.fieldColumn(model_field)
                items[field_column].setText(model.getProperty(model_field))
