...
class LocationSettingDialog(QDialog):
    def __init__(self):
        ...
        first_location_lbl = MyDefaultWidgets.basicQLabel(text='지역', alignment=Qt.AlignLeft)
        second_location_lbl = MyDefaultWidgets.basicQLabel(text='근무지', alignment=Qt.AlignLeft)
        self.first_location_le = MyDefaultWidgets.basicQLineEdit(alignment=Qt.AlignRight)
        self.second_location_le = MyDefaultWidgets.basicQLineEdit(alignment=Qt.AlignRight)
        ...
        location_button = MyDefaultWidgets.basicQPushButton(text='지역 입력하기')
    ...
    def locationButtonClicked(self) -> None:
        if not (self.first_location_le.text() or self.second_location_le.text()):
            ...
            if reply == MyMessageBox.Yes:
                ConfigModule.Application.setMajorLocation(head_location)
                ConfigModule.Application.setMinorLocation(tail_location)
                self.close()
            else:
                return